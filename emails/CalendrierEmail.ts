...
function stepCard(step: CalendrierStep, last = false): string {
  ...
  return `
    <div style="border-left:3px solid ${color};padding:16px 16px 16px 20px;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;margin-bottom:${last ? '0' : '8px'}">
      ${step.isArrivee
        ...
        : ''}
      <p style="margin:0 0 4px;font-family:${FONT};font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:${color}">${step.mois}</p>
      <p style="margin:0 0 8px;font-family:${FONT};font-size:14px;font-weight:700;color:#ffffff;line-height:1.3">${step.action}</p>
      <p style="margin:0${step.lien ? ' 0 10px' : ''};font-family:${FONT};font-size:12px;color:rgba(255,255,255,0.6);line-height:1.65">${step.description}</p>
      ${step.lien
        ? `<a href="${step.lien.url}" style="font-family:${FONT};font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:${color};text-decoration:none">${step.lien.label} →</a>`
        : ''}
    </div>
  `;
}
  ...
  const content = `
    <div style="padding:24px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;margin-bottom:28px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
        <span style="font-size:36px">${paysEmoji}</span>
        <div>
    ...
    <p style="text-align:center;margin:14px 0 0;font-family:${FONT};font-size:12px;color:rgba(255,255,255,0.4)">
      Le PDF est joint à cet email · <a href="${SITE}/checklist" style="color:rgba(77,143,255,0.7);text-decoration:none">Télécharge aussi la checklist PDF</a>
    </p>
  `;
